...
const localStorage = new LocalStorage("./subs_db");
...
// ========================================================
const publicVapidKey = "BNQutWeuwdJ-2_SK02czaqoWaXFDVzbdYUYX7c0KZumJSm2K5gg3PwYc41ftfaFIuojnIOLRcE-h7IZiTupFBiU"; 
const privateVapidKey = "Kh0dbAIO3luhGkdZ4mpE2L6yqGVkzSD9e17jL1m-IKM";

webpush.setVapidDetails(
  "mailto:chatgptprokotas02@gmail.com", 
  publicVapidKey,
  privateVapidKey
);

// ============================
// CACHE (Código original mantido)
// ============================
let cache = {
  data: null,
  timestamp: null,
...
// ============================
// ÚLTIMO STATUS VÁLIDO (Código original mantido)
// ============================
...
// ============================
// HELPERS (Código original mantido)
// ============================
...
// ============================
// BUSCAR OCORRÊNCIAS (Código original mantido)
// ============================
...
async function fetchCCMOccurrences() {
  ...
  if (!url.startsWith("http")) return new Map();
...
// ============================
// FETCH CCM/ARTESP (Código original mantido)
// ============================
async function fetchCCMStatus() {
  ...
  for (const el of headingEls) {
    ...
    if (isUnavailableStatusText(statusText)) {
      ...
      if (isLastGoodValid(lastGoodEntry)) {
        ...
        if (!info.updated) info.updated = lg.updatedInfo || "";
      } else {
  ...
  if (finalLines.length === 0) throw new Error("Nenhuma linha encontrada no CCM (layout pode ter mudado)");
  return finalLines;
}
...
// ======================================================
// 👇 AQUI COMEÇA O CÓDIGO DO ROBÔ DE NOTIFICAÇÃO
// ======================================================

// Rota 1: Salva quem quer receber (Frontend manda pra cá)
...
// Rota 2: Serve o arquivo sw.js para o navegador
...
// ======================================================
// 👇 ROBÔ VIGIA DE NOTIFICAÇÃO (Roda a cada 60s)
// ======================================================
let pushMemory = {}; 

setInterval(async () => {
  try {
    // ⚠️ REUTILIZA SUA FUNÇÃO ORIGINAL getData()
    const data = await getData(); 
    ...
    data.lines.forEach(line => {
      ...
      // Se é a primeira vez, só memoriza
      if (!pushMemory[num]) {
      ...
      if (pushMemory[num] !== status) {
        console.log(`🔔 NOTIFICAÇÃO: Linha ${num} mudou para ${status}`);
        
        const payload = JSON.stringify({
          title: `Linha ${num}: ${status}`,
          body: line.reason || "Toque para ver detalhes no site.",
        ...
        localStorage._keys.forEach(key => {
            try {
                ...
                webpush.sendNotification(sub, payload).catch(e => {
                    // Remove usuário se ele não existe mais
...
app.listen(PORT, () => {
  ...
  console.log("Rotas: / | /api/status | /subscribe | /sw.js");
});
